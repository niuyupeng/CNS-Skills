...
- “完善摘要、引言和讨论，目标是顶刊投稿。”
- “审计这篇综述，把泄漏到正文里的证据链、框架等编辑脚手架找出来。”
- “设计或审计论文图、表、图注和图形摘要。”

启用隐式调用后，不必背固定提示词。仓库提供了 64 条中英文[正向与负向触发评测](evals/README.md)，其中 30 条为锁定的留出集；它检查“论文润色、中译英、审稿回复、顶刊顶会、图表审计”是否容易触发，同时避免把单纯找文献、仅改参考文献格式、普通文案、中枢神经系统知识问答或规避 AI 检测误路由到 CNS Skills。留出集不能用来堆触发关键词，评测也不代表所有宿主版本的调用保证。
...
脚本会提示高风险主张、数字附近缺少可识别引用、重复开头、套话、对称句式与 DOI 解析结果；还会检查方括号数字编号引文（如 `[1]`、`[1, 2–4]`）的缺失、未引用、重复、断号、乱序和章节覆盖，比较中英文稿中的数字、单位、统计量、引用和图表编号，并检查图表“有引用无图注 / 有图注无引用”。当前引文审计器不解析作者—年份制或上标 Vancouver 引文。它们是透明的分诊工具，不是 AI 检测器，也不能判断引文是否真正支持句子、证明翻译等价或代替逐篇阅读原始文献。完整 JSON 可能包含本地路径和未发表稿件片段，应按保密文件处理；对外分享时使用 `--shareable` 去除这些字段。

0.6.0 版新增“**综述—Skill 双向迭代**”流程：先从论证缺口而不是目标篇数扩充文献；候选来源依次经过发现、元数据核验、全文定位、支持关系核验、发表状态检查和作者确认；再分别检查引文存在性、元数据/状态、支持关系、范围、位置和独立性。体内验证也不会自动被写成高自主等级：只有体内终点真正反馈到下一轮自主选择时，才可能满足项目对 `C4` 的相应定义。

0.5.0 版引入了“**编辑脚手架留在幕后**”的综述自然度审计。主张账本、证据卡、证据链、比较框架和证据剖面可以作为作者侧分析工具，但除非学科本身需要这些术语，否则不应反复进入成稿。正文应直接写清研究对象、材料、测量、结果、比较和局限。审计采用上下文规则，不会因为出现“数据集”、某个有名称的报告框架或具体实验依据，就机械地把正常科研用语判成问题。

## 顶刊顶会语料基线与图表能力

0.3.0 版加入、0.6.0 版继续保留了可复现的 320 篇英文摘要基线：Cell、Nature、Science、AAAI、CVPR、NeurIPS、ICML、ICLR 各 40 篇。分析器只输出聚合指标和题名/标识符清单，不保存摘要全文；会议年份覆盖差异也会明确披露。这些统计用于检查写作判断，不是照抄模板或录用预测器。
...
项目只从公开 Skill 与科研工具中提炼可泛化原理，并在[开源调研记录](research/open-source-skill-review.md)中逐项保留许可证边界；不复制第三方提示词、代码、模板或资产。公开可见不等于开源可复用，尤其是带专有条款的 source-available 文档 Skill。
