...
0.5.0 版新增“**编辑脚手架留在幕后**”的综述自然度审计。主张账本、证据卡、证据链、比较框架和证据剖面可以作为作者侧分析工具，但除非学科本身需要这些术语，否则不应反复进入成稿。正文应直接写清研究对象、材料、测量、结果、比较和局限。审计采用上下文规则，不会因为出现“数据集”、某个有名称的报告框架或具体实验依据，就机械地把正常科研用语判成问题。

## 顶刊顶会语料基线与图表能力

0.3.0 版加入、0.5.0 版继续保留了可复现的 320 篇英文摘要基线：Cell、Nature、Science、AAAI、CVPR、NeurIPS、ICML、ICLR 各 40 篇。分析器只输出聚合指标和题名/标识符清单，不保存摘要全文；会议年份覆盖差异也会明确披露。这些统计用于检查写作判断，不是照抄模板或录用预测器。
